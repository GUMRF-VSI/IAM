...
class TokenLogic:
    def generate_token(self, user: models.User) -> auth.schemas.TokenData:
        return auth.schemas.TokenData(access=self.create_access_token(user=user),
    ...
    def create_access_token(user: models.User,
        ...
        access_token = auth.schemas.AccessTokenData(exp=expire_timestamp, sub=user.get_sub())
    ...
    def create_refresh_token(user: models.User):
        refresh_token = auth.schemas.RefreshData(id=user.id, email=user.email)
    ...
    def create_identity_token(user: models.User):
        ...
        identity_token = auth.schemas.RefreshData(id=user.id, email=user.email)
