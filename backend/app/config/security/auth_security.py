...
class TokenLogic:
    def generate_token(self, user: models.User) -> schemas.TokenData:
        return schemas.TokenData(access=self.create_access_token(user=user),
    ...
    def create_access_token(user: models.User,
        ...
        access_token = schemas.AccessTokenData(exp=expire_timestamp, sub=user.get_sub())
    ...
    def create_refresh_token(user: models.User):
        refresh_token = schemas.RefreshData(id=user.id, email=user.email)
    ...
    def create_identity_token(user: models.User):
        ...
        identity_token = schemas.RefreshData(id=user.id, email=user.email)
